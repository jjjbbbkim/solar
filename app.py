...
st.caption("실제 계산은 아래에서 입력한 SMP/REC 단가 1회 값으로 20년 동일 단가로 계산합니다.")
...
st.caption(
    ...
    f"물가 반영률 {OM_INFL:.1f}%를 연차별로 적용합니다."
)
...
if st.button("계산하기"):
    ...
    for year in range(1, int(operation_years) + 1):
        ...
        # 유지비(원): 매출 × 4% × 물가
        maintenance_won = annual_revenue_won * (OM_RATE / 100) * ((1 + OM_INFL / 100) ** (year - 1))
    ...
    # ✅ 예전 방식: 연차를 index로 둬서 왼쪽 0,1,2... 인덱스가 안 생김
