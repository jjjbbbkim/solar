...
if area_input_type == "평":
    area = st.sidebar.number_input("면적(평)", min_value=1, step=1)  # 정수만
    area_m2 = area * 3.3
else:
    area_m2 = st.sidebar.number_input("면적(㎡)", min_value=1, step=1)  # 정수만
    area = int(area_m2 / 3.3)

# ===== 3. 발전용량 계산 =====
if plant_type == "노지":
    capacity = int((area / 3000) * 1000)  # kW, 정수
    rec_weight = 1.0
else:
    capacity = int((area / 2000) * 1000)  # kW, 정수
    rec_weight = 1.5
...
loan_amount = total_cost * (1 - self_ratio/100)  # 만원 기준
